...
def parse_frontmatter(content):
    ...
    if match:
        ...
        i = 0
        stack = [{'data': frontmatter, 'indent': -1}]  # Stack to track nested structures
        ...
        while i < len(lines):
            ...
            # Handle simple value
                ...
                if not value and i < len(lines) and (len(lines[i]) - len(lines[i].lstrip())) > indent:
                    current[key] = {}
...
def md_to_html(md_content):
    ...
    # Code blocks with language (allow non-greedy matching)
    html = re.sub(r"```(\w+)(.*?)?```", lambda m: f'<pre><code class="language-{m.group(1)}">{escape(m.group(2))}</code></pre>', html, flags=re.DOTALL)

    # Code blocks without language
    html = re.sub(r"```(.*?)```", lambda m: f'<pre><code>{escape(m.group(1))}</code></pre>', html, flags=re.DOTALL)
    ...
    # Images ![alt](url)
    html = re.sub(r"!\[([^\]]*)\]\(([^\)\s]+)\)(?!\{)", r'<img src="\2" alt="\1" class="img-fluid" loading="lazy">', html)
    html = re.sub(r"!\[([^\]]*)\]\(([^\)\s]+)\)\{[^}]*width=(\d+)%[^}]*\}", r'<img src="\2" alt="\1" class="img-fluid" style="width: \3%;" loading="lazy">', html)
    html = re.sub(r"!\[([^\]]*)\]\(([^\)\s]+)\)\{[^}]*\}", r'<img src="\2" alt="\1" class="img-fluid" loading="lazy">', html)
            ...
            if post_dir.is_dir():
                ...
                if post_file.exists():
                    with open(post_file, 'r', encoding='utf-8') as f:
                        ...
                        if frontmatter.get('title', '').lower() in [section, f'{section.capitalize()}', 'book reviews']:
                            continue
...
def generate_post_page(post, output_dir):
    ...
    html_content = re.sub(r'src="images/([^"]+)"', f'src="{post["section"]}_{post["slug"]}_images/\\1"', html_content)
    ...
    html = generate_html_layout(f"{post['title']} - Thomas W. Bush", page_content, 'post')
...
def generate_listing(title, posts, output_path, page_type='listing'):
    ...
    if page_type != 'home' and posts:
        ...
        if categories:
            ...
            for cat in categories:
                options.append(f'<option value="{cat}">{cat.replace("-", " ").title()}</option>')
            filter_html = f"""
    ...
    for post in posts:
        ...
        categories_html = "".join([f"<span>{cat}</span>" for cat in post["categories"][:3]])
...
def generate_about_page(base_dir, output_dir):
    ...
    if link_html:
        social_links_html = '<div class="social-links">' + ' '.join(link_html) + '</div>'
